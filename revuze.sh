...
cleanup() {
  local env=$1
  echo "Starting cleanup process for Revuze ($env environment)..."

  # Stop all running Revuze containers
  echo "Stopping all Revuze containers..."
  docker stop mongo_container client server > /dev/null 2>&1

  # Remove all Revuze containers
  echo "Removing all Revuze containers..."
  docker rm mongo_container client server > /dev/null 2>&1

  # Remove all Revuze images
  echo "Removing all Revuze Docker images..."
  ...
  # Optionally remove the Revuze network
  echo "Removing the Revuze Docker network..."
  docker network rm $NETWORK_NAME > /dev/null 2>&1

  echo "Cleanup complete. All Revuze containers, images, and volumes have been removed."
}
